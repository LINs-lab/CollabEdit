...
from experiments.py.eval_utils_mcfConflict import compute_rewrite_quality_mcfConflict
...
DS_DICT = {
    ...
    "mcf_conflict": (McfConflictDataset, compute_rewrite_quality_mcfConflict)
}
...
def main(
    ...
    if "whetherConflict" in conflict_exp_type or "_A" in conflict_exp_type:
        print("Load A")
        ds_A = ds_class(DATA_DIR, tok=tok, size=num_edits, exp_type="_A")
        print("Load B")
        ds_B = ds_class(DATA_DIR, tok=tok, size=num_edits, exp_type="_B")
        ds = []
        ds.extend(ds_A)
        Rec_chunks = chunks(ds, num_edits)
    elif "_B" in conflict_exp_type or "A+B" in conflict_exp_type:
        ds_B = ds_class(DATA_DIR, tok=tok, size=num_edits, exp_type="_B")
        ds_A = ds_class(DATA_DIR, tok=tok, size=num_edits, exp_type="_A")
        ds = []
        ds.extend(ds_A)
        ds.extend(ds_B)
        Rec_chunks = chunks(ds, num_edits * 2)
    elif "single_B" in conflict_exp_type:
        ds = []
        print("Load B")
        ds_B = ds_class(DATA_DIR, tok=tok, size=num_edits, exp_type="_B")
        ds.extend(ds_B)
    ...
    for record_chunks in Rec_chunks:
        print("record_chunks len: " + str(len(record_chunks)))
        ...
        for record in record_chunks:
            if not Path(
                    case_result_template.format(
                        num_edits // 2, record["case_id"])
            ).exists():
        ...
        if "global" in merging_method:
            start = time()
            delta_Nx10, covs, __, ks = apply_memit2model_modified(
                model,
                tok,
                [
        ...
        with torch.no_grad():
            ...
            if "global" in merging_method:
                ...
                for w_name, (key_mat, val_mat) in delta_Nx10.items():
            ...
            # N x 10
                ...
                for w_name, delta_demo in upd_matrix_dict_list[0].items():
                    ...
                    if "task" in merging_method:
                        ...
                        for i in range(10):
                            delta_5x10 = delta_5x10 + \
                                upd_matrix_dict_list[i][w_name].to("cuda")
                        ...
                        cov = covs[w_name].to("cuda")
                        A = deepcopy(cov)
                        for i in range(10):
                            all_kkt = all_kkts[i][w_name].to("cuda")
                            delta_5x10 = delta_5x10 + upd_matrix_dict_list[i][w_name].to("cuda") @ (
                                all_kkt + cov)
                            A = A + all_kkt
                            upd_matrix_dict_list[i][w_name].cpu()
                            all_kkt.cpu()
                    ...
                    delta_5x10 = upd_matrix_match_shape(delta_5x10, w2.shape)
                    w2[...] += delta_5x10.float()

                    # clear GPU memory
                    delta_demo = delta_demo.cpu()
                    delta_5x10 = delta_5x10.cpu()
                    del delta_demo, delta_5x10
                    torch.cuda.empty_cache()
        ...
        if "whetherConflict" in conflict_exp_type or "A+B" in conflict_exp_type:
            ds_B_chunks = chunks(ds_B, num_edits)
            eval_chunks = ds_B_chunks
        elif "_B" in conflict_exp_type or "_A" in conflict_exp_type:
            Rec_chunks = chunks(ds_A, num_edits)
            eval_chunks = Rec_chunks

        for eval_chunk in eval_chunks:
            print("eval_chunk's len: " + str(len(eval_chunk)))
            for record in eval_chunk:
                metrics1 = {}
                metrics2 = {}
                print("record: " + str(record["case_id"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ds_name",
        choices=["mcf", "cf", "zsre", "mcf_conflict"],
        default="mcf",
    ...
    parser.add_argument(
        ...
        type=str,
        default="global",
        help="has ['Perfomrance_[A+B]', 'whetherConflict','Performance_A','Performance_B']",
    )
