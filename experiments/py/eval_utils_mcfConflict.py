...
def compute_rewrite_quality_mcfConflict(
...
def test_batch_prediction(
    ...
    for i in range(logits.size(0)):
        ...
        for j in range(cur_len):
            ...
            b = logits[i, prefix_lens[i // 2] + j - 1, :]
            b1 =logits[i, prefix_lens[i // 2] + j - 1, :][6983]
            c1 = logits[i, prefix_lens[i // 2] + j - 1, :][4141]
            probs[i] += -torch.nn.functional.log_softmax(
                logits[i, prefix_lens[i // 2] + j - 1, :], dim=0
            )[cur_tok].item()
            idx = logits[i, prefix_lens[i // 2] + j - 1, :].argmax().item()
            answer.append(tok.decode(idx))
        probs[i] /= cur_len
